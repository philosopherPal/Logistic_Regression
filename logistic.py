...
def multinomial_train(X, y, C, 
    ...
    """
    def softmax_func(x):
        n = np.exp(x - np.amax(x))
        d = np.sum(n, axis=1)
        return (n.T/d).T

    yOneHot = np.eye(C)[y]
    for i in range(max_iterations):
        fW = softmax_func((w.dot(X.T).T + b)) - yOneHot
        gradW = fW.T.dot(X)/N
        gradb = np.sum(fW, axis=0)/N
        w = w - step_size * gradW
        b = b - step_size * gradb
    assert w.shape == (C, D)
    assert b.shape == (C,)
...
def multinomial_predict(X, w, b):
    ...
    def softmax_func(x):
        ...
        d = np.sum(x, axis=1)
        return (x.T/d).T
    preds = softmax_func((w.dot(X.T)).T + b)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    yOneHot = np.eye(C)[y]
    for i in range(max_iterations):
        fW = sigmoid((w.dot(X.T)).T + b) - yOneHot
        gradW = fW.T.dot(X)/N
        gradb = np.sum(fW, axis=0)/N
        w = w - step_size * gradW
        b = b - step_size * gradb
